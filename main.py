...
DDURC_DD_HEADER = [
        "No", "Application Code", "As of Date",
        "Business Term",
...
def refresh_db_object():
	"""
        This will fetch the data from data source id \
                and all the prodution table from DBC
        """

        iprint("refreshing database object, please wait...")

        myTDConnection = TDConnection(
                DSN='prod', DBName='bip_vtdb')
        sql_command = """
...
def getOIAFileCandidate():
    ...
    DatasourceTemplate = map(
            lambda x: 'DD_' + str(x[0])+'_'+x[1]+""".xls""",
            dataSourceIdList)
    ...
    for myPath, myFileName in oiaFileList:
        try:
            excelDoc = ExcelDoc(
                    ...
                    headerLoc=2,
                    mode='r')
            valResult = validate_schema(
                    OIA_DD_HEADER,
                    excelDoc.header)
            head = zip(*valResult)
            if not all(head[1]):
                raise ExcelHeaderError(
                        "the header of File of [{}] \
                                is not up the standard, \
                                please check".format(myPath))

        except ExcelHeaderError as e:
            eprint(e.args)
            eprint("There is issue with the header of \
                    File [{}], it will not \
                    be processed".format(myPath))

            continue
        except xlrd.XLRDError as e:
            eprint(e.args)
            eprint("There is issues with File of [{}], \
                    it might be get corrupted, \
                    it will not be processed".format(myPath))
            continue
        ...
        for item in data:
            if not item[0] or not item[1]:
                ...
                break
        if empty:
            eprint("table name of physical name in OIA \
                    file [{}] is empty, please check, \
                    this file will not be \
                    processed".format(myPath))
            continue
...
def refresh_oia_cdm_mapping():
    ...
    fileList = walk(
            CDM_MAPPING_DIR,
            r"""(\AT\d{4}[X]?_.*?)_C\.xls[x]?$""")
    iprint("the size of oia to cdm \
            mapping [{0:4d}]".format(len(fileList)))
    fullDict = {}
    ...
    for myPath, myFileName in fileList:
        try:
            ...
            if not all(head[1]):
                raise ExcelHeaderError(
                        "the header of File {} have \
                                issue".format(myPath))

        except ExcelHeaderError as e:
            eprint(e.args)
            eprint("the header of File {} \
                    will not be processed".format(myPath))
            continue
        except xlrd.XLRDError as e:
            eprint(e.args)
            eprint("the header of File {} will not be \
                    processed".format(myPath))
            continue
        finally:
            pass
...
def create_ddurc_urc():
    iprint("Creating DDURC - User Reference Code, \
            Please wait...")
    ...
    for row in refTableList:
        ...
        if referenceTableName in newTotalPivot:
            ...
            try:
                ...
                if len(fetchRow[0]) > 2:
                    iprint("table of [{}] have columns other \
                            than code and desc, it will not \
                            inlcuded as not considered as \
                            reference data".format(row[1]))
                    continue
                if descReg.search(fetchRow[0][0]):
                    map(itemgetter(1, 0), fetchRow)
                # fetchRow = [[
                # mapping[referenceTableName],
                # f[0], f[1] ]for f in fetchRow]
                fetchRow1 = []
                for f in fetchRow[1:]:
                    if f[1] is None or f[1] == 'None':
                        f[1] = f[0]
                        fetchRow1.append([
                            mapping[referenceTableName].upper(),
                            f[0],
                            f[1]])
...
def create_ddurc_cdm_dd():
    iprint('Creating DDURC CDM Data Dictionary,\
            Please wait...')
    fileName = CDM_DD_FILE_NAME
    data = csv_reader(fileName)
    ...
    prodTableList = csv_reader(CDM_PRODUCTION_FILE_NAME)
    iprint("total number of [{}] production \
            tables retrieved".format(len(prodTableList)))
    dddict = {}
    ddList = []
    descReg = re.compile(
            r"""Data Management.*?Start.*?\n(.*)\n.*Data Management.*End""", re.DOTALL)
...
class Application(tk.Frame):
    ...
    def createWidgets(self):
        ...
        self.var.set("Welcome to DD URC Wizard, \
                please select the module to proceed")
        self.L1 = tk.Label(
                self,
        ...
        self.b2 = tk.Button(
                self,
                text="Cancel", width=10, command=quit)
