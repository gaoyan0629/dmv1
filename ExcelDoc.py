    ...
    def __init__(
        ...
        if len(suffix) != 1:
            raise fileSuffixError(
                    "Error",
                    "Suffix of file of {} is wrong".format(
                        fileName))
        ...
        if self.mode == 'r':
            if self.fileSuffix == '.xls':
                self.workBook = xlrd.open_workbook(
                        self.fileName)
                self.workSheet = self.workBook.sheet_by_name(
                        self.tabName)
            ...
            if self.fileSuffix == '.xls':
                ...
                for row in range(len(data)):
                    for col in range(len(data[row])):
                        ...
                        self.workSheet.cell(
                                row=row + 1,
                                column=col + 1).value = data[row][col]
