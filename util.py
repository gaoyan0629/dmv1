...
def validate_schema (source,target):
	"""
	this checks if the value defined in source are in target or not. if found, it also put the position (pos)
	"""
	ret = []
	for i in range(len(source)):
		...
		for k in range(len(target)):
			if ((isinstance(source[i],unicode) or isinstance(source[i],str))
			  and (isinstance(target[k],unicode) or isinstance(target[k],str))):
				if source[i].strip().lower() == target[k].strip().lower():
					found = True
					pos = k
					break
...
def walk(dirname,pattern):	
	"""
	this is the customized version of python walk, it search the root directory of dirname based on the pattern
...
def dedup ( data, keyCol):
	"""
	this will de-duplicate from the list in data based on the key defined in data col
	"""
	sourcedict = {}
	retlist = []
	...
	for key, value in sorted(sourcedict.iteritems(), key=lambda (k,v): (v[0],k)):
...
def lookup(source, lookup, sourcecol, lookupcol):
	"""
	this will look up from list in (lookup) and return its position in relation to where it lookup 
	"""
	sourcekey = [] 
	lookupkey = []
...
def iprint(*args, **kwargs):
    print('['+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S") +']'+'[INFO]', *args,**kwargs)
...
class fileSuffixError(Exception):
     "this is exception when the file suffix of excel file is wrong"
     pass
class NoSupportError(Exception):
     "this is exception when the using the feature that have not been supported"
     pass
...
def netDriveConn():
	...
	subprocess.call(r"""net use Y: "http://teamsite.1bank.dbs.com/sites/EnterpriseServices/BDWAnalyst/BIP Design Documents/CDM Design Documents" """,shell=True)
